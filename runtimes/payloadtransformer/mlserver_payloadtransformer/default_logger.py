...
async def index(request: Request):
    ...
    if type_header is None:
        logger.error("Type header not found")
    ...
    if body_length and int(body_length) > int(max_payload_bytes):
        too_large_message = (
            ...
            + index_name
            + "/"
            + (log_helper.DOC_TYPE_NAME if log_helper.DOC_TYPE_NAME is not None else "_doc")
            + "/"
            + request_id
            + " adding "
            + message_type
    ...
    try:
        ...
        return Response(added_content, 200)
    except Exception:
        traceback.print_exc()
    sys.stdout.flush()
...
async def metadata(request: Request):
    try:
        ...
        serving_engine = request.query_params.get('serving_engine', 'SeldonDeployment')
        if not serving_engine:
            serving_engine = 'SeldonDeployment'

        namespace = request.query_params.get('namespace', 'seldon')
        name = request.query_params.get('name', 'sklearn')
        predictor = request.query_params.get('predictor', 'default')
